<nav role='navigation' class='superhero-nav superhero-nav-blog hide-for-print'>
  <div id="nav-toggle-container">
    <a href="http://www.thomaswicker.com/" class="nav-current-page">
      Thomaswicker.com
    </a>

    <div id="nav-toggle">
      <i class="fa fa-bars"></i>
    </div>
  </div>

  <ul>
    <li>
      <a href='http://www.thomaswicker.com/'>
        Home
      </a>
    </li>
    <li>
      <a href='http://www.thomaswicker.com/portfolio/'>
        Portfolio
      </a>
    </li>
    <li>
      <a href='http://www.thomaswicker.com/photography/'>
        Photography
      </a>
    </li>
    <li class="active">
      <a href='http://www.thomaswicker.com/culinary/'>
        Culinary
      </a>
    </li>
    <li>
      <a href='http://www.thomaswicker.com/my-resume/'>
        Resume
      </a>
    </li>
    <li>
      <a href='http://www.thomaswicker.com/contact/'>
        Contact
      </a>
    </li>
  </ul>
</nav>
